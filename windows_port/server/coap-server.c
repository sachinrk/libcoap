...
static BOOL
handle_sigint(DWORD SigType) {
  UNREFERENCED_PARAMETER(SigType);
...
hnd_get_index(coap_context_t *ctx UNUSED_PARAM,
  ...
  coap_add_data(response, (unsigned int)strlen(INDEX), (unsigned char *)INDEX);
}

static void
hnd_put_index(coap_context_t *ctx,
  ...
  Payload = coap_new_string(size);

  assert(Payload != NULL);

  strncpy_s((char*)Payload->s, size + 1, (const char*)data, size);
  ...
  coap_add_data(response, (unsigned int)strlen("LIBCOAP-Windows10"), (unsigned char *)"LIBCOAP-Windows10");

  fprintf(stdout, "Payload recevied:%s", Payload->s);

  coap_delete_string(Payload);
  ...
  if (my_clock_base) {
    ...
    if (request != NULL
        ...
        min(5, COAP_OPT_LENGTH(option))) == 0) {
          /* output ticks */
          len = _snprintf_s((char *)buf,
                             sizeof(buf),
                             min(sizeof(buf), response->max_size - response->length),
                             "%u",
                             (unsigned int)now);

          coap_add_data(response, (unsigned int)len, buf);
...
static int
join(coap_context_t *ctx, __in_bcount_z(NI_MAXHOST) char group_name[NI_MAXHOST]){
...
__cdecl wmain(
    __in int argc,
    __inout_ecount(argc) PSTR argv[]
    )
{
  coap_context_t  *ctx;
  char group[NI_MAXHOST] = {0};
  fd_set readfds;
  ...
  while ((opt = getopt(argc, ConvertedArgs, "A:g:p:v:", sizeof("A:g:p:v:"))) != -1) {
    switch (opt) {
    case 'A' :
      strncpy_s(addr_str, sizeof(addr_str), optarg, NI_MAXHOST-1);
      addr_str[NI_MAXHOST - 1] = '\0';
      break;
    case 'g' :
      strncpy_s(group, sizeof(group), optarg, NI_MAXHOST-1);
      break;
    case 'p' :
  ...
  if (!SetConsoleCtrlHandler((PHANDLER_ROUTINE)handle_sigint, TRUE))
  {
    return 0;
  }
