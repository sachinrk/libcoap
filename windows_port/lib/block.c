...
coap_write_block_opt(coap_block_t *block, unsigned short type,
  ...
  start = (size_t)block->num << (block->szx + 4);
  ...
  if (want <= avail) {
	...
	return -3;
      ...
      debug("decrease block size for %zu to %d\n", avail, coap_fls((unsigned int)avail) - 5);
      szx = block->szx;
      block->szx = coap_fls((unsigned int)avail) - 5;
      block->m = 1;
...
coap_add_block(coap_pdu_t *pdu, unsigned int len, const unsigned char *data,
  ...
  size_t start;
  start = (size_t)block_num << (block_szx + 4);

  if (len <= start)
    return 0;

  return coap_add_data(pdu,
		       (unsigned int)min(len - start, (size_t)1 << (block_szx + 4)),
		       data + start);
}
#endif /* WITHOUT_BLOCK  */
