...
print_timestamp(__deref_inout_bcount_part(len) char *s, size_t len, coap_tick_t t) {
  struct tm *tmp;
...
static inline size_t
print_timestamp(__inout_bcount(len) char* s, size_t len, coap_tick_t t) {
...
print_readable(
        ...
        unsigned int len,
		__out_bcount_part(buflen, return + 1) unsigned char *result,
		unsigned int buflen,
		int encode_always ) {
...
msg_code_string(uint8_t c) {
  ...
  if (c < sizeof(methods)/sizeof(char *)) {
    ...
    _snprintf_s(buf, sizeof(buf), sizeof(buf), "%u.%02u", c >> 5, c & 0x1f);
    return buf;
  }
}
...
msg_option_string(uint16_t option_type) {
  ...
  _snprintf_s(buf, sizeof(buf), sizeof(buf), "%u", option_type);
  return buf;
}

static unsigned int
print_content_format(unsigned int format_type,
		     __out_bcount(buflen) unsigned char *result, unsigned int buflen) {
  struct desc_t {
    unsigned int type;
    const char *name;
  };
  ...
  for (i = 0; i < sizeof(formats)/sizeof(struct desc_t); i++) {
    if (format_type == formats[i].type) {
      return _snprintf_s((char *)result, buflen, buflen, "%s", formats[i].name);
    }
  }

  /* unknown content format, just print numeric value to buf */
  return _snprintf_s((char *)result, buflen, buflen, "%d", format_type);
}
...
coap_show_pdu(const coap_pdu_t *pdu) {
  ...
  while ((option = coap_option_next(&opt_iter))) {
    ...
    case COAP_OPTION_BLOCK2:
      ...
      buf_len = _snprintf_s((char *)buf, sizeof(buf), sizeof(buf), "%u/%c/%u",
    ...
    case COAP_OPTION_SIZE1:
      ...
      buf_len = _snprintf_s((char *)buf, sizeof(buf), sizeof(buf), "%u",
  ...
  if (coap_get_data((coap_pdu_t *)pdu, &data_len, &data)) {
	...
	fprintf(COAP_DEBUG_FD, "%02x", *data++);
      ...
      if (print_readable(data, (unsigned int)data_len, buf, sizeof(buf), 0)) {
