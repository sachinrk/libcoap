...
coap_new_error_response(coap_pdu_t *request, unsigned char code,
  ...
  coap_opt_iterator_t opt_iter;
  coap_pdu_t *response;
  size_t size = COAP_HDR_SIZE + request->hdr->token_length;
  int type;
  coap_opt_t *option;
  ...
  response = coap_pdu_init((unsigned char)type, (unsigned char)code, request->hdr->id, size);
  if (response) {
    /* copy token */
...
coap_wellknown_response(coap_context_t *context, coap_pdu_t *request) {
  ...
  if (coap_get_block(request, COAP_OPTION_BLOCK2, &block)) {
    debug("create block\n");
    offset = (size_t)block.num << (block.szx + 4);
  ...
  *resp->data = COAP_PAYLOAD_START;
  ...
  resp->length += (unsigned short) (COAP_PRINT_OUTPUT_LENGTH(result));
  return resp;

 error:
  /* set error code 5.03 and remove all options and data from response */
  resp->hdr->code = COAP_RESPONSE_CODE(503);
  resp->length = (unsigned short)(COAP_HDR_SIZE + resp->hdr->token_length);
  return resp;
}
...
no_response(coap_pdu_t *request, coap_pdu_t *response) {
  ...
  if (nores) {
    val = (uint8_t)(coap_decode_var_bytes(coap_opt_value(nores), coap_opt_length(nores)));
  }
...
handle_request(coap_context_t *context, coap_queue_t *node) {
  ...
  if (h) {
    ...
    response = coap_pdu_init(node->pdu->hdr->type == COAP_MESSAGE_CON ? COAP_MESSAGE_ACK : COAP_MESSAGE_NON,
                             0,
                             node->pdu->hdr->id,
                             COAP_MAX_PDU_SIZE);
