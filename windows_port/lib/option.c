...
coap_option_iterator_init(coap_pdu_t *pdu, coap_opt_iterator_t *oi,
  ...
  oi->next_option = (unsigned char *)pdu->hdr + COAP_HDR_SIZE
  ...
  assert((COAP_HDR_SIZE + pdu->hdr->token_length) <= pdu->length);

  oi->length = pdu->length - (COAP_HDR_SIZE + pdu->hdr->token_length);
...
__success(return != 0)
size_t
coap_opt_encode(__inout_bcount_part(maxlen, return) coap_opt_t *opt, size_t maxlen, unsigned short delta,
