...
append_to_output(const unsigned char *data, size_t len) {
  ...
  if (file == NULL) {
    if (!output_file.s || (output_file.length && output_file.s[0] == '-'))
      ...
      if ((file = fopen((char *)output_file.s, "w")) == NULL) {
        perror("fopen");
        return -1;
      }
    }
...
coap_new_request(coap_context_t *ctx,
  ...
  pdu->hdr->token_length = (unsigned short)the_token.length;
  ...
  if (length) {
    if ((flags & FLAGS_BLOCK) == 0)
      ...
      coap_add_block(pdu, (unsigned int)length, data, block.num, (unsigned char)block.szx);
  }
...
static int
resolve_address(const str *server, struct sockaddr *dst, const unsigned short Family) {

  struct addrinfo *res, *ainfo;
  struct addrinfo hints;
  static char addrstr[256] = {0};
  int error, len=-1;

  memset(addrstr, 0, sizeof(addrstr));
  if (server->length && server->length < sizeof(addrstr))
  ...
  for (ainfo = res; ainfo != NULL; ainfo = ainfo->ai_next) {
    ...
    case AF_INET6:
        ...
        memcpy(dst, ainfo->ai_addr, sizeof(struct sockaddr_in6));

        if (len != ainfo->ai_addrlen) {
          fprintf(stderr, "getaddrinfo: Look it up\n");
        }
    ...
    case AF_INET:
        ...
        memcpy(dst, ainfo->ai_addr, sizeof(struct sockaddr_in));
...
static void
cmdline_content_type(__in_z char *arg, unsigned short key) {
  ...
  while (q && *q && (valcnt < sizeof(value) / sizeof(value[0]))) {
    p = strchr(q, ',');

    if (isdigit((unsigned char)*q)) {
      if (p)
        *p = '\0';
      value[valcnt++] = (unsigned char)atoi(q);
    } else {
      for (i=0;
...
static int
cmdline_blocksize(__in_z char *arg) {
  unsigned short size;
...
set_blocksize(void) {
  ...
  if (method != COAP_REQUEST_DELETE) {
    ...
    block.m = (opt == COAP_OPTION_BLOCK1) &&
      ((size_t)1 << (block.szx + 4) < payload.length);
...
static void
cmdline_subscribe(__in_z char *arg UNUSED_PARAM) {
    ...
    if (!ipv6_delimiter) {
      if (proxy_port_str == strchr((const char *)arg, ':')) {
        ...
        *proxy_port_str++ = '\0'; /* split */
        proxy_port = (unsigned short)atoi(proxy_port_str);
      }
    } else {
...
static inline void
cmdline_token(__in_z char *arg) {
  strncpy_s((char *)the_token.s, sizeof(_token_data), arg, min(sizeof(_token_data), strlen(arg)));
  the_token.length = strlen(arg);
}

static void
cmdline_option(__in_z char *arg) {
  unsigned int num = 0;
  ...
  coap_insert(&optlist,
              new_option_node((unsigned short)num, (unsigned int)strlen(arg), (unsigned char *)arg));
}
...
static int
cmdline_input(__in_z char *text, str *buf) {
  int len;
...
static int
cmdline_input_from_file(__in_z char *filename, str *buf) {
  FILE *inputfile = NULL;
  ssize_t len;
  int result = 1;
  ...
  len = (ssize_t)fread(buf->s, 1, buf->length, inputfile);
...
static method_t
cmdline_method(__in_z char *arg) {
  static char *methods[] =
...
get_context(const char *node, const char *port) {
  ...
  if ( s != 0 ) {
    fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(s));
    fprintf(stderr, "GetLastError = %d", WSAGetLastError());
    return NULL;
  }
    ...
    )
{
  ...
  char ConvertedArgs[20][256] = {0};

  //
  // Convert argv strings from widechar to mbcs.
  //


  if (argc > sizeof(ConvertedArgs) / sizeof(ConvertedArgs[0]))
  {
    return -1;
  }
  ...
  for (i = 0; i < argc; i++)
  {
    wcstombs_s(&tempBufferSize,
               ConvertedArgs[i],
               sizeof(ConvertedArgs[i]),
               (PWSTR)argv[i],
               sizeof(ConvertedArgs[i]) - sizeof('\0'));
  }

  if ((s = WSAStartup(MAKEWORD(2, 2), &Data)) != NO_ERROR)
  {
      coap_log(LOG_WARNING, "coap_new_endpoint: WSAStartup failed");
      fprintf(stderr, "wmain: %s\n", gai_strerror(s));
      return NULL;
  }

  while ((opt = getopt(argc,
                       ConvertedArgs,
                       "Na:b:e:f:g:m:p:s:t:o:v:A:B:O:P:T:G:",
                       sizeof("Na:b:e:f:g:m:p:s:t:o:v:A:B:O:P:T:G:"))) != -1) {
    switch (opt) {
    case 'a' :
      strncpy_s(node_str, sizeof(node_str), optarg, NI_MAXHOST-1);
    ...
    case 'f' :
      ...
      break;
    case 'p' :
      strncpy_s(port_str, sizeof(port_str), optarg, NI_MAXSERV-1);
    ...
    case 'v' :
      ...
      break;
    case 'G' :
      if (strcmp(optarg, "AF_INET6") == 0)
      {
        Family = AF_INET6;
      }
      else if (strcmp(optarg, "AF_INET") == 0)
      {
        Family = AF_INET;
      }
      break;
  ...
  res = resolve_address(&server, &dst.addr.sa, Family);
  ...
  else if (dst.addr.sa.sa_family == AF_INET6)
  {
      dst.addr.sin6.sin6_port = htons(port);
      dst.size = sizeof(SOCKADDR_IN6);
  }
  ...
  if ((pdu = coap_new_request(ctx, method, &optlist, payload.s, payload.length)) == NULL)
    return -1;
  ...
  while ( !(ready && coap_can_exit(ctx)) ) {
    ...
    result = select((int)ctx->sockfd + 1, &readfds, 0, 0, &tv);
